                    ...
                    <div class="menu-pcmenu-container">
                        <ul id="menu-pcmenu" class="down-menu nav-menu sf-js-enabled sf-arrows">
                            ...
                            <c:forEach items="${allCategoryList}" var="category">
                                <!--只有当categoryPid=0时才会被当作主菜单，其余的菜单出现在下拉列表中-->
...
<div id="search-main">
    <div class="searchbar">
        <form method="get" id="searchform" action="/search" accept-charset="UTF-8">
            <scan>
                <input type="text" value="" name="keywords" id="s" placeholder="输入搜索内容">
                sorted by:
    ...
    <%--进行热点搜索关键词的设置--%>
    <h2>热点关键词：</h2>
    <c:forEach items="${allHotKeywords}" var="k">
<%--            <form  action="/search" accept-charset="UTF-8">--%>
<%--                <button type="submit" value="${k}" name="keywords">${k}</button>--%>
<%--            </form>--%>
        <a href="/search?keywords=${k.keyword}">
            <span class="comment">&nbsp;&nbsp;&nbsp;${k.keyword}</span>
        </a>
    </c:forEach>
